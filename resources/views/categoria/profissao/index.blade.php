<?php
    ...
    $tituloPagina = "Categoria Profissão"
?>
  ...
  <div class="c">
    ...
    <div class="conteudo">
      <div class="Cabeçalho">
        <h1></h1>
        <h1>{{$tituloPagina}}</h1>
        <a href='{{route('categoria.profissao.novo')}}'>
              ...
              <thead>
                <tr id='id{{ $profissao->id }}'>
                  ...
                  <td class='text-center'>
                    <div>
                      <form action="{{ route('categoria.profissao.destroy', ['id'=>$profissao->id])}}" method="post" >
                        @csrf
                        @method("DELETE")
                        <?php echo(str_replace("row#","row" . $profissao->id,$buttonLixeira))?>
                      </form>
                      <form action="{{ route('categoria.profissao.edit', ['id'=>$profissao->id])}}" method="get" >
                        @csrf

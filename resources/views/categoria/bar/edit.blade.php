...
<x-layout-dashboard title="Categoria"  css="resources/css/dashboard/categoria.css">
    ...
    <div class="container">
        <h1>{{ $TituloPagina}}</h1>
        <form action="{{ route('categoria.bar.update', $categoria_bar->id) }}" method="post">
            @csrf
            @method("PUT")
            <label for="nome">Nome</label>
            <input type="text" id="nome" name="nome" required value="@if(isset($categoria_bar))<?php echo($categoria_bar->nome)?> @endif">
            <label for="descricao">Descrição</label>
            <textarea  required id="descricao" name="descricao">@if(isset($categoria_bar))<?php echo($categoria_bar->descricao)?> @endif</textarea>

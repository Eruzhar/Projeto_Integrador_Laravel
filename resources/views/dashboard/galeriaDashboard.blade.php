            ...
            <div class='card' style='width: 22rem;' id='card{{$post->id}}'>
                ...
                <div class='card-body'>
                    ...
                    <div class='button'>
                        ...
                        <form action="{{ route('destroyGaleria', ['id'=>$post->id])}}" method="post" >
                            @csrf
                            @method("DELETE")
                            <?php echo( str_replace( "#$#","row" . $post->id,$buttonLixeira));?>
                        </form>
                        <form action="{{ route('editGaleria', ['id'=>$post->id])  }}" method="post">
                            @csrf
                            @method("PUT")
                            <?php echo( str_replace( "#$#", $post->id,$buttonVisivel));?>
                        </form>
                        <form action="{{ route('editGaleria', ['id'=>$post->id])  }}" method="post">
                            @csrf
                            @method("PUT")
                            <?php echo( str_replace("#$#", $post->id, $buttonOcultar));?>
                        </form>
                    </div>
                </div>
            </div>

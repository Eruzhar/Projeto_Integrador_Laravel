        ...
        <div class="sidebar">
            <a href='{{ route('menu') }}'>
                <button type="button" class="btn btn-primary" data-bs-toggle="tooltip" data-bs-placement="top"
                    data-bs-custom-class="custom-tooltip" data-bs-title="Voltar">
        ...
        <div class="conteudo">
            ...
            <div class="form-outline" data-mdb-input-init>
                <div class="Filtro">
                    <h4>Profissão:</h4>
                    <form method="get">
                        <select class="form-control" id="exampleFormControlSelect1" name="selecao">
                            <option value="all">Todas profissões</option>
                            @if(isset($profissoes))
                                @foreach ($profissoes as $profissao)                        
                                    <option value="p{{$profissao->id}}">{{$profissao->nome}}</option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <form method="get">
                        <select class="form-control" id="exampleFormControlSelect1" name="ordem">
                            ...
                            <option value="profissao_id">Profissão</option>
                        </select>
                        <x-button-enviar descricao="Ordenar "></x-button-enviar>
                    </form>
                            ...
                            <thead>
                                <tr id='id"+json[i].indice+"'>
                                    ...
                                    <td class='text-center'>
                                        <div>
                                            <form action="{{ route('curriculo.destroy', ['id' => $curriculo->id])}}"
                                                method="post">
                                                @csrf
                                                @method("DELETE")
